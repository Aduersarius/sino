...
swiftc -parse-as-library -O -module-name Sino \
  ...
  -framework SwiftUI -framework AppKit -framework IOKit -framework Combine -framework ServiceManagement -framework SystemConfiguration -framework CoreWLAN -framework CoreLocation \
  -o "$BIN" \
...
ROOTAPP="$ROOT/Sino.app"
rm -rf "$ROOT/Pulse.app" "$ROOT/dist/Pulse.app"
